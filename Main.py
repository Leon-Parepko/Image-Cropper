...
def process(cwd, border, color_rgb, split):
    ...
    for file in os.listdir(cwd):
        if file.endswith(".jpg") or file.endswith(".png"):
            ...
            splitted_img = split_img(img, split)
...
confirm_button = tk.Button(window, text='Process', width=25, command=lambda: process(dir_entry.get(), border_slider.get(), [blue_slider.get(), green_slider.get(), red_slider.get()], [split_H_slider.get(), split_V_slider.get()]))
